...
#define SL3_0 26
#define SL3_1 28
const uint8_t solPin[6] = { SL1_0, SL1_1, SL2_0, SL2_1, SL3_0, SL3_1 };
...
void sendData() {
  ...
  static uint8_t lastGrip = 255, lastSph = 255, lastBot = 255, lastTop = 255;
  ...
  // Cek data berubah
  bool dataChanged = (_grip_sph != lastGrip) || (_arm_sph != lastSph) || (_arm_box_bot != lastBot) || (_arm_box_top != lastTop);
  ...
  if ((dataChanged && !waitingACK) || (waitingACK && millis() - lastSend >= RETRY_INTERVAL)) {
    ...
    if (dataChanged && !waitingACK) {
      ...
      uint8_t checksum = 0;
      for (int i = 1; i <= 4; i++) checksum ^= packet[i];
      packet[5] = checksum;
      packet[6] = 0x55;
