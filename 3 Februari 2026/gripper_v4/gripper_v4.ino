...
HardwareSerial Serial3(PB11, PB10);  // RX only, TX disabled
...
#define CONV_MAJU PA3
#define CONV_MUNDUR PA2
// ===================== 74HC4067 ==========================
#define MUX_SIG PA4
#define MUX_S0 PB12
#define MUX_S1 PB13
...
void setup() {
  ...
  pinMode(CONV_MAJU, OUTPUT);
  pinMode(CONV_MUNDUR, OUTPUT);
...
void motorFuzzy(int target, int current,
  ...
  pwm = constrain(50 + pwm, MIN_PWM, MAX_PWM);
  ...
  // Apply to motor
  if (fuzzyOutput > 0) { 
    analogWrite(pinCW, fuzzy.lastPWM);
    analogWrite(pinCCW, 0);
  } else if (fuzzyOutput < 0) {
    analogWrite(pinCCW, fuzzy.lastPWM);
    analogWrite(pinCW, 0);
  } else {
    analogWrite(pinCW, 0);
    analogWrite(pinCCW, 0);
...
// =========================================================
void motor360(int input, uint8_t pinCW, uint8_t pinCCW) {
  int diff = input - 90;
  ...
  if (diff > 0) {
    analogWrite(pinCW, pwm);
    analogWrite(pinCCW, 0);
  } else {
    analogWrite(pinCCW, pwm);
    analogWrite(pinCW, 0);
  }
}
  ...
  while (Serial3.available()) {
    ...
    switch (state) {
      ...
      case 3:  // Read footer
        if (b == 0x55) {
          // Data OK
          tgtGrip = constrain(data[0], 0, 180);
          tgtSph = constrain(data[1], 0, 180);
          tgtBot = constrain(data[2], 0, 180);
          tgtTop = constrain(data[3], 0, 180);
          tgtConveyor = constrain(data[4], 0, 180);
          tgt6 = constrain(data[5], 0, 180);
          dataTerima = true;
          Serial3.write(0xAA);  // ACK
        }
        state = 0;
        break;
    }
...
void loop() {
  ...
  motor360(tgtGrip, GRIPPER_CLOSE, GRIPPER_OPEN);
  motor360(tgtConveyor, CONV_MAJU, CONV_MUNDUR);  
